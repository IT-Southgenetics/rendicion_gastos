...
/**
 * Cierra la rendición como aprobada (workflow + status) y dispara el webhook de n8n.
 * Requiere que todos los gastos del reporte estén en estado `approved`.
 */
export async function runReportApprovedClosure(
  ...
  const hasAnyNotApproved = expenseList.some((e) => e.status !== "approved");
  ...
  if (webhookUrl) {
    ...
    const currencies = new Set(
      expenseList.map((e) => (e as { currency?: string }).currency ?? "UYU"),
    );
    const isMulticurrency = currencies.size > 1 || (currencies.size === 1 && !currencies.has(budgetCurrency));

    const expenseDetails = expenseList.map((e: { id: string; amount: number; currency?: string | null; description?: string | null; category?: string | null; merchant_name?: string | null; expense_date?: string | null }) => ({
      id: e.id,
  ...
  revalidatePath(`/dashboard/aprobador/reports/${reportId}`);
}
